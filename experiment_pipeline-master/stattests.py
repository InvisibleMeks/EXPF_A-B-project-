...
class DataByGroups:
    def __init__(self, df_0: DataFrame, df_1: DataFrame):
        self.df_0 = df_0
        self.df_1 = df_1      
...
class TestByData(abc.ABC):
    @abc.abstractmethod
    def __call__(self, DataByGroups) -> EstimatorCriteriaValues:
        pass  
...
class Mann_Whitney_U_test(TestByData):

    def __call__(self, groups: DataByGroups) -> EstimatorCriteriaValues:
        try:
            U, pvalue = mannwhitneyu(groups.df_0, groups.df_1, alternative = "two-sided")
        except Exception as e:
            cfg.logger.error(e)
            U, pvalue = None, None

        return EstimatorCriteriaValues(pvalue, U)
    
    
class Z_test(TestByData):

    def __call__(self, groups: DataByGroups) -> EstimatorCriteriaValues:
        try:
            Z, pvalue = ztest(groups.df_0, groups.df_1, alternative = "two-sided")
        except Exception as e:
            cfg.logger.error(e)
            Z, pvalue = None, None

        return EstimatorCriteriaValues(pvalue, Z)
...
def data_by_groups(df):
    _variants = df[cfg.VARIANT_COL].unique()
    df_0 = df.loc[df[cfg.VARIANT_COL] == _variants[0], 'l_ratio']
    df_1 = df.loc[df[cfg.VARIANT_COL] == _variants[1], 'l_ratio']
    return DataByGroups(df_0, df_1)